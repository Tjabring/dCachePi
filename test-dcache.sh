...
json_response=$(curl -s -k -u admin:$PASSWD -X POST \
  -H 'Content-Type: application/macaroon-request' \
  https://localhost:2881)

# Extract macaroon
macaroon=$(echo "$json_response" | grep -o '"macaroon": "[^"]*"' | sed 's/"macaroon": "\(.*\)"/\1/')
# Extract targetWithMacaroon
target_uri=$(echo "$json_response" | grep -o '"targetWithMacaroon": "[^"]*"' | sed 's/"targetWithMacaroon": "\(.*\)"/\1/')

echo "Macaroon: $macaroon"
echo "Target URI: $target_uri"
sleep $SLEEP

# Macaroon WebDAV upload
run_test "Macaroon WebDAV upload" \
  "curl -s -H \"Authorization: Bearer $macaroon\" -T README.md http://localhost:2880/home/tester/macaroon-README.md"

# Macaroon WebDAV download
run_test "Macaroon WebDAV download" \
  "curl -s -H \"Authorization: Bearer $macaroon\" -o /tmp/macaroon-README.md http://localhost:2880/home/tester/macaroon-README.md"

# Macaroon WebDAV PROPFIND check
run_test "Macaroon WebDAV PROPFIND" \
  "curl -s -H \"Authorization: Bearer $macaroon\" -X PROPFIND http://localhost:2880/home/tester/ | grep macaroon-README.md"

echo
echo "Requesting macaroon (READ, WRITE, LIST on /home/tester)..."
json_response=$(curl -s -k -u admin:$PASSWD -X POST \
  -H 'Content-Type: application/macaroon-request' \
  -d '{"caveats":[{"type":"activity","value":"READ"},{"type":"activity","value":"WRITE"},{"type":"activity","value":"LIST"},{"type":"path","value":"/home/tester"}]}' \
  https://localhost:2881)
